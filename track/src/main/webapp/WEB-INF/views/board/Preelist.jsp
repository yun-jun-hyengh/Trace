	...
	<div class="container">
		<div class="row">
			...
			<div class="col-lg-8">
				...
				<h2 class="page-header"><span style="color: #ff52a0;"></span>자유게시판
	             ...
	             <table class="table table-hover">
	             	...
	             	<tr>
	             		<td>${bo.num}</td>
						<td>${bo.writer}</td>
						<td><a href="${cpath}/board/Preeread/${bo.num}">${bo.title}</a></td>
	             ...
	             <div class="paging">
					...
					<c:if test="${pageMaker.prev}">
						<a href="Preelist${pageMaker.makeSearch(pageMaker.startPage - 1)}">◀</a>
					</c:if>
					<c:forEach begin="${pageMaker.startPage}" end="${pageMaker.endPage}" var="idx">
						<strong><a href="Preelist${pageMaker.makeSearch(idx)}">${idx}</a></strong>
					</c:forEach>
					<c:if test="${pageMaker.next && pageMaker.endPage > 0}">
						<a href="Preelist${pageMaker.makeSearch(pageMaker.endPage + 1)}">▶</a>
					</c:if>
					</ul>
				</div>
				<br><br>  
	 ...
	 <div class="search">
	   <select class="form-control" name="searchType">                            	
		...
		<a href="${cpath}/board/Preewrite" class="btn btn-primary">글쓰기</a>
		</span>
</header> 
</form>
...
$(function(){
    $('#searchBtn').click(function() {
      self.location = "Preelist" + '${pageMaker.makeQuery(1)}' + "&searchType=" + $("select option:selected").val() + "&keyword=" + encodeURIComponent($('#keywordInput').val());
    });
  }); 
